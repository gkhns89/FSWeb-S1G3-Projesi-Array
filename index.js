...
function kopyala(lezzetlerdizisi) {
  /*kod buraya*/
  return lezzetlerdizisi;
}
...
function dizi25Cesitmi(orijinalTatlar) {
  /*kod buraya*/
  let count = orijinalTatlar.length;
...
function cesitEkle(orijinalTatlar, aroma) {
  /*kod buraya*/
  orijinalTatlar.unshift(aroma);
...
function ismeGoreCesitCikar(orijinalTatlar, cikacakTat) {
  /*kod buraya*/
  orijinalTatlar.splice(orijinalTatlar.indexOf(cikacakTat), 1);
...
function ismeGoreFiltrele(orijinalTatlar, filtre) {
  ...
  for (let i = 0; i < orijinalTatlar.length; i++) {
    if (orijinalTatlar[i].includes(filtre)) {
      ozelTatlar.push(orijinalTatlar[i]);
    }
  }
  return ozelTatlar;
}
...
function ortalamaKelimeSayisi(orijinalTatlar) {
  ...
  let checkedArray = orijinalTatlar.values();

  for (const value of checkedArray) {
    let check = value.split(" ").length;
